...
exports.appUpdate = (req, res, next) => {
    ...
    app.findOne({ app_id: validateResult.value.appId }, { _id: 1 })
        .then(appInfo => {
            ...
            app.updateOne({ user_id: req.user._id, app_name: req.body.appName, app_active: false }, {
...
exports.getAppStatus = (req, res, next) => {
    ...
    app.findOne({ user_id: req.user._id, app_name: req.query.appName }, { app_active: 1, _id: 0 })
        .then(appInfo => {
            if (!appInfo) {
                return res.json({
                    success: false,
...
exports.appStatusChange = (req, res, next) => {
    let app = new model('app')
    app.findOne({ user_id: req.user._id, app_name: req.body.appName }, { app_active: 1, app_id: 1, password: 1, _id: 0 })
        .then(appInfo => {
            ...
            app.updateOne({ user_id: req.user._id, app_id: appInfo.app_id }, { 'app_active': !appInfo.app_active })
                .then(updateData => {
                    if (!updateData.result.nModified) {
                        return res.status(200).json({
                            success: false,
