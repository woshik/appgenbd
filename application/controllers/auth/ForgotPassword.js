...
const { checkEmail, checkUser } = require(join(MODEL_DIR, 'auth/Model_Forgot_Password'))
    ...
    checkEmail(validateResult.value.email)
        .then(({ success, info }) => {
            if (success) {
                ...
                return res.json({ success: true, url: `${web.accountActivation.url}?email=${encodeURIComponent(validateResult.value.email)}&rd=${info.userRDId}` })
            } else {
