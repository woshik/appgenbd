								...
								if (isMatch) {
									if (!!user.account_active) {
										...
										if (!!user.userRDId && checkRDParam(user.userRDId)) {
											return resolve({
												success: false,
												info: `Your account not active. To active your account <a href="${accountActivation.url}?email=${encodeURIComponent(email)}&rd=${
													user.userRDId
												}">click here</a>`
											});
										} else {
											updateRDParam(userCollection, user._id)
												.then(rd => {
													return resolve({
														success: false,
														info: `Your account not active. To active your account <a href="${accountActivation.url}?email=${encodeURIComponent(
															email
														)}&rd=${rd}">click here</a>`
													});
												})
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (!userData || !!userData.account_disable) {
				...
				userData.account_limit_available = dateTime.subtract(new Date(userData.account_activation_end_date), dateTime.addHours(new Date(), 6)).toDays() >= 0;
			}
