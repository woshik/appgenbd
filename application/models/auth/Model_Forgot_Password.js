...
exports.checkEmail = email => {
    return new Promise((resolve, reject) => {
        ...
        db.createCollection('users')
            .then(userCollection => {
                userCollection.findOne({ email: email }, { projection: { account_active: 1, token_refresh: 1, userRDId: 1 } })
                    .then(user => {
                        if (!user) {
                            resolve({ success: false, info: 'Account not found.' })
                            ...
                            resolve({ success: true, info: userRDId })
                        }
                    })
        ...
        db.createCollection('users')
            .then(userCollection => {
                userCollection.findOne({ email: email, userRDId: rd }, { projection: { account_active: 1, token_refresh: 1 } })
                    .then(user => {
                        if (!user) return resolve({ success: false, info: 'Account not found. Try again.' })
                        ...
                        return resolve({ success: true, info: null, sendCode: tokenTime ? 'Please, check your email account.' : tokenTime })
                    })
                    .catch(err => reject(err))
            })
            .catch(err => reject(err))
